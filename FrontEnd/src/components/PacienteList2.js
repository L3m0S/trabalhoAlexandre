...
export default function PacientesList2() {
  ...
  async function getData() {
    try { // tenta buscar os dados
      ...
      if(response.data.length > 0) setPacientes(response.data)
    }
    catch(error) {
      console.error(error)
    }
  ...
  return (
    <>
      ...
      <Paper elevation={4}>
        <DataGrid className={classes.dataGrid} rows={pacientes} columns={columns} pageSize={10} autoHeight={true} disableSelectionOnClick={true} />
      </Paper>
    </>
  )
}