...
export default function PacientesList() {
  ...
  async function getData() {
    try { // tenta buscar os dados
      let response = await axios.get('http://localhost:3333/internacao')
      let newList = []

      response.data.forEach(element => {
          const id = element.int_codigo
          delete element['int_codigo']

          let newElement = {id: id, ...element}
          newList.push(newElement)
      })
      console.log(newList)
      if(response.data.length > 0) setInternacao(newList)
    }
    catch(error) {
      console.error(error)
    }
  ...
  async function deleteItem() {
    try {
      await axios.put(`http://localhost:3333/internacao/status/${deletable}`)
  ...
  const columns = [
    ...
    { 
      ...
      flex: true,
      renderCell: params => (
        <IconButton aria-label="editar" onClick={() => history.push(`/edit-new-internacao/${params.id}`)}>
          <EditIcon />
        </IconButton>
      )
    },
  ...
  return (
    <>
      ...
      <h1>Listagem de Internação</h1>
      <Toolbar className={classes.toolbar}>
        <Button color="secondary" variant="contained" size="large" 
          startIcon={<AddBoxIcon />} onClick={() => history.push('/new-internacao')}>
          Nova Internação
        </Button>
      </Toolbar>
      <Paper elevation={4}>
        <DataGrid className={classes.dataGrid} rows={internacao} columns={columns} pageSize={10} autoHeight={true} disableSelectionOnClick={true} />
      </Paper>
    </>
  )
}