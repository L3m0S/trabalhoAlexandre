...
export default function InternacaoForm() {
  ...
  useEffect(() => {
    ...
    if(params.int_codigo) {
      setTitle('Editar Paciente')
      getData(params.int_codigo)
    }
  }, [])

  async function getData(id) {
    try {
      let response = await axios.get(`http://localhost:3333/internacao/${id}`)
  ...
  function handleInputChange(event, prop) {
    ...
    if(event.target.id) {
      ...
      pacienteTemp[prop] = event.target.value
    }
    setPaciente({...pacienteTemp, [prop]: event.target.value})
  ...
  async function saveData() {
    try {
      ...
      paciente.int_codigo = params.id
      if(params.id) await axios.put(`http://localhost:3333/internacao`, paciente) 
      
      // Senão, estaremos criando um novo registro, e o verbo HTTP a ser usado é o POST
      else await axios.post('http://localhost:3333/internacao', paciente)
  ...
  function handleSbClose() {
    ...
    if(sbStatus.severity === 'success') history.push('/internacao')
  }
  ...
  function handleDialogClose(result) {
    ...
    if(result) history.push('/guiainternacao')
  }
  ...
  return (
    <>
      ...
      <form className={classes.form} onSubmit={handleSubmit}>
        ...
        <TextField 
          id="int_codigocid" 
          label="Codigo da doença" 
          variant="filled"
          onChange={handleInputChange}
          required  /* not null, precisa ser preenchido */
          placeholder="Informe o codigo da doença!"
          fullWidth
        >
        </TextField>
