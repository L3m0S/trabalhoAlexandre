...
export default function GuiaexamesList() {
  ...
  async function getData() {
    try { // tenta buscar os dados
      let response = await axios.get('http://localhost:3333/guiaexame')
      let newList = []

      response.data.forEach(element => {
          const id = element.gui_codigo
          delete element['gui_codigo']

          let newElement = {id: id, ...element}
          newList.push(newElement)
      })
      console.log(newList)
      if(response.data.length > 0) setGuiaexame(newList)
    }
    catch(error) {
      console.error(error)
    }
  ...
  async function deleteItem() {
    try {
      await axios.put(`http://localhost:3333/guiaexame/status/${deletable}`)
  ...
  const columns = [
    ...
    { 
      ...
      flex: true,
      renderCell: params => (
        <IconButton aria-label="editar" onClick={() => history.push(`/edit-new-guia/${params.id}`)}>
          <EditIcon />
        </IconButton>
      )
    },
  ...
  return (
    <>
      <ConfirmDialog isOpen={dialogOpen} onClose={handleDialogClose}>
        Deseja realmente excluir esta guia de exame?
      </ConfirmDialog>
      ...
      <h1>Listagem de Guias de exame</h1>
      <Toolbar className={classes.toolbar}>
        <Button color="secondary" variant="contained" size="large" 
          startIcon={<AddBoxIcon />} onClick={() => history.push('/new-guia')}>
          Nova Guia de exame
        </Button>
      </Toolbar>
      <Paper elevation={4}>
        <DataGrid className={classes.dataGrid} rows={guiaexame} columns={columns} pageSize={10} autoHeight={true} disableSelectionOnClick={true} />
      </Paper>
    </>
  )
}